...
DATASET_FILENAME = 'pluginfile.txt'
...
def calc_loglikelihood(X, priors, means, covars):
    ...
    means = np.array(means)
    covars = np.array(covars)
    # covars = np.array(covars) + np.eye(len(covars))*COV_BIAS
...
def EM_algo(X, k, maxiter=100, thresh=1e-4, animate=False):
    """
    Fit GMM
    :param X:  Data matrix. (n x m) = (#ofsamples x #offeatures)
    :param k:  #ofClusters
    ...
    for i in range(maxiter):
        ...
        if m <= 2 and animate:
            #     plotting start ---------------------------------------------->>>
            axis.cla()
            plot_contours(X, means, covars, 11, title='test')
            time.sleep(2)
            fig.canvas.draw()
...
if __name__ == '__main__':
    ...
    for k in range(2, max_k + 1):
        ...
        print(out['means'])
        #
        # plot_contours(X, out['means'], out['covariances'], 11, title=str(k)+'.png')
    ...
    animate_k = int(input('k for animation:'))

    EM_algo(X, animate_k, maxiter=100, animate=True)